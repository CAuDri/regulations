...
The vehicle drives in the right lane.

\subsection{Scenario}

The complexity of this scenario is limited.
% It consists of a road with two parallel lanes - one for each driving direction.
This scenario shall imitate a rural road environment, consisting of long straight sections, tight turns, intersections, and side road junctions.
...
\label{event_lane_markings}

% Both lanes are separated by a dashed center line.
% The center line is interrupted every 200 mm for another 200 mm.
...
% In this case the solid lines are spaced approx.
% 20 mm apart, yielding a total marking width of approx. 56 mm to 60 mm.
...
Cancelling an attempt is penalized (cf. Section \ref{freedrive_scoring}).
...
f the vehicle does not return into the right driving lane on its own, RC-mode must be activated immediately.
...
Parking has been moved to the Obstacle Evasion Course.
...
Timing for the event starts with opening the start box gate described in Section \ref{start_box}.
\HighlightNew{After the 2 minutes have passed, the vehicle must be brought to an immediate and complete stop by switching to RC-mode.
