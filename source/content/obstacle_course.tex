...
% Changing lanes is only allowed with an empty left lane, oncoming traffic must have completely passed.
...
\begin{highlight}
	...
	% \HighlightNew{Each obstacle course element may only be evaluated once per round.
	% 	Elements may not be retried using RC-mode.

	Elements of the Obstacle Evasion Course may not be retried using RC-mode.
	% If incorrect behavior regarding an element occurs before the activation of RC-mode, the element will be evaluated based on that behavior.
	...
	\HighlightFix{Check if this definition is sufficient}
...
% 	% 	If incorrect behavior regarding an element occurs before RC-mode activation, the element will be evaluated based on that behavior.
% 	% An element may only be continued after RC-mode use if the RC-mode activation was not caused by incorrect behavior related to that element.
